...
try {
  ...
  if ($keyword <> "" ) {
    $sql = "(SELECT reg_no, first_name, email, last_name, profile_image, contact FROM student_patient WHERE 1 AND "
            . " (first_name LIKE :keyword) ORDER BY first_name) "
            . "UNION ALL (SELECT  reg_no, first_name, email, last_name, last_name, contact  FROM staff_patient WHERE 1 AND "
            . " (first_name LIKE :keyword) ORDER BY first_name )";
    $stmt = $DB->prepare($sql);
    ...
    $sql = "(SELECT reg_no, first_name, last_name, email, profile_image , contact FROM student_patient) union (SELECT reg_no, first_name, last_name, email, profile_image, contact FROM staff_patient) ORDER BY first_name ";
    $stmt = $DB->prepare($sql);
  }
...
<?php
  if ($_GET["msg"] == "success") {
    echo successMessage("Email has been sent successfully");
  ...
  <div class="panel panel-primary">
    ...
    <div class="panel-body">
      ...
      <div class="col-lg-12" style="padding-left: 0; padding-right: 0;" >
        <form action="all_patient.php" method="get" >
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                ?>
                <li><a href="all_patient.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php
              }
            }
            ?>
...
<div class="col-md-4">
  ...
  <?php } else {?>
<div class="col-lg-12 center alert-danger"> Access to this page is denied.</div>
</div>
    <?php
  }
      include 'footer.php';
