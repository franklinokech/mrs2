...
$page_limit = ($_GET["show"] <> "" && is_numeric($_GET["show"]) ) ? $_GET["show"] : 19;
...
try {
  ...
  if ($keyword <> "" ) {
    ...
    $sql = "SELECT * FROM staff WHERE 1 ORDER BY active = 'YES' ";
    $stmt = $DB->prepare($sql);
  }
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                ?>
                <li><a href="active.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php
              }
            }
            ?>
