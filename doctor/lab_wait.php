          ...
          <?php foreach ($results as $lab) {
           ...
           ?>
					<td style="text-align: center;"><?php echo $lab["visit_no"]; ?></td>
								  ...
								  <td><a  href="view_patients_result.php?reg_no=<?php echo $lab["reg_no"]; ?> && visit_no=<?php echo $lab["visit_no"]; ?>"><?php echo ucwords($pati["first_name"]. " ". $pati['last_name']); ?></a></td>
                  ...
                  <?php
                    ...
                    $d= $lab["date"];
                    $age = date_diff(date_create($d), date_create($date1))->i;
                  ?>						 
							  <td><?php echo($age ." ". 'mins Ago'); ?></td>
            </tr>
            <?php }; ?>
          </tbody></table>
        </div>
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                ?>
                <li><a href="lab_wait.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php
              }
            }
            ?>
